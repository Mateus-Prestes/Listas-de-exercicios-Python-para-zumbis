...
def soma(n1, n2):
  ...
  for i in range(min(len(n1), len(n2))):
      if n1[i] + n2[i] < 9:
        final.append(n1[i] + n2[i] + acumulado)
        acumulado = 0
      else:
        final.append(n1[i] + n2[i] - 10 + acumulado)
        acumulado = 1
  if acumulado != 0:
      final.append(1)
  return final
...
def anagrama(s1, s2):
  for x in range(len(s1)):
    if s1[x] in s2 and s2[x] in s1 and len(s1) == len(s2):
      return True
    else:
      return False
